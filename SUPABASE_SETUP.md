...
2. Create a server-controlled instructor-offer coupon that is exactly 100 percent off and repeats for exactly 2 months. Prefer a Stripe-enforced restriction to the configured Hire Line Dancers Product. If the Stripe account does not retain `applies_to.products`, bind the account-scoped coupon with the exact `managed_by`, `intended_product_id`, and `restriction_mode` metadata expected by the application. The approval path still verifies one subscription item on the exact configured Product and Price before applying the coupon. The coupon must have no `redeem_by` date and no `max_redemptions` limit because first-100 allocation, invitation timing, and one-time eligibility are enforced by the application. Do not create or distribute a customer-facing promotion code for this coupon. Do not allow another promotion to stack with it.
3. Save the Product's `prod_...` identifier as `STRIPE_PRODUCT_ID`, the Price's `price_...` identifier as `STRIPE_PRICE_ID`, and the instructor-offer coupon's identifier as `STRIPE_INSTRUCTOR_OFFER_COUPON_ID`. Set `STRIPE_EXPECTED_MODE=live` for production. The approval function retrieves the coupon and fails closed unless its mode, Stripe-enforced or server-verified Product binding, percentage, duration, and redemption settings match the exact requirements above.
...
The dedicated Customer Portal configuration is required in production and optional for local or sandbox use. Valid Terms, Privacy, and support URLs are configured in Stripe Public details, and production uses `STRIPE_REQUIRE_TERMS_CONSENT=true`. Production Checkout fails closed when this setting is missing, invalid, or false. The live Stripe secrets are installed in Supabase. On August 12, 2026, the configured instructor-offer coupon and matching `STRIPE_INSTRUCTOR_OFFER_COUPON_ID` secret were verified in the live Hire Line Dancers account as valid, bound through server-verified metadata to the $14.99 monthly instructor membership Product, 100 percent off for exactly 2 months, without an expiration or redemption cap, and without a promotion code. The approval function additionally enforces one subscription item on that exact Product and Price.
